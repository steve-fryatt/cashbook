...
char *templates_get_current_menu_name(char *buffer)
{
	...
	*buffer = '\0';
	...
	else if (menu_up == templates_menu_list[TEMPLATES_MENU_MAIN])
		strcpy(buffer, "MainMenu");
	else if (menu_up == templates_menu_list[TEMPLATES_MENU_SORDER])
		strcpy(buffer, "SorderMenu");
	else if (menu_up == templates_menu_list[TEMPLATES_MENU_PRESET])
