...
wimp_menu *analysis_template_menu_build(struct file_block *file, osbool standalone)
{
	...
	for (line = 0; line < file->saved_report_count; line++) {
		...
		strcpy(analysis_template_menu_entry_link[line].name, file->saved_reports[line].name);
		if (!standalone)
			strcat(analysis_template_menu_entry_link[line].name, "...");
...
analysis_t analysis_template_menu_decode(wimp_selection *selection)
{
