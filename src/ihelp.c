...
void ihelp_set_modifier(wimp_w window, char *modifier)
{
	...
	win_data = ihelp_find_window(window);

	if (win_data != NULL)
		strcpy(win_data->modifier, modifier);
}
	...
	*buffer = '\0';
	...
	if (window == wimp_ICON_BAR) {
		...
		*icon_name = '\0';
		...
		if (*icon_name != '\0') {
			snprintf(token, sizeof(token), "Help.%s%s.%s", win_data->name, win_data->modifier, icon_name);
		...
		if (!found) {
			snprintf(token, sizeof(token), "Help.%s%s", win_data->name, win_data->modifier);
			found = msgs_lookup_result(token, help_text, IHELP_LENGTH);
		}
